...
np.fill_diagonal(Lamd_q, [4, 4, 1])
np.fill_diagonal(Phi_q, [4, 4, 1])
...
class Controller:
    ...
    def rpyRate(self):
        ...
        self.tou_q = (2/np.absolute(self.avg_thrust))*self.tou_q
        print(self.tou_q)
        print("-----------")
    ...
    def average(self, value):
        ...
        num_array = [0,0,0,0,0,0] #take 10 elements
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        Thrust = cnt.average(cnt.tou_p[2])
        # movement_cmd.body_rate.x = cnt.average(-cnt.tou_q[0])
        # movement_cmd.body_rate.y = cnt.average(-cnt.tou_q[1])
        # movement_cmd.body_rate.z = cnt.average(-cnt.tou_q[2])

    	# movement_cmd.thrust = cnt.tou_p[2]
        movement_cmd.thrust = Thrust
        cnt.getAvgThrust(Thrust)
        # print(cnt.tou_p)
