...
class Controller:
    ...
    def __init__(self):
        ...
        self.Lam = np.array([2.0, 2.0, 4.0])
        self.Phi = np.array([1.2, 1.2, 0.6])
        self.M = 0.1
        self.alpha_0 = np.array([5,5,5])
    ...
    def multiDoFCb(self, msg):
		...
		self.sp.pose.orientation.x = pt.transforms[0].rotation.x
		self.sp.pose.orientation.y = pt.transforms[0].rotation.y
		self.sp.pose.orientation.z = pt.transforms[0].rotation.z
		self.sp.pose.orientation.w = pt.transforms[0].rotation.w
    ...
    def vector2Arrays(self, vector):
        array = np.empty(0)
        array = np.append(array, vector.x)
        array = np.append(array, vector.y)
        array = np.append(array, vector.z)
        try:
        	array = np.append(array, vector.w)
    ...
    def geo_con(self):
        des_th = self.th_des()
        # print(self.vector2Arrays(self.sp.pose.orientation))
        des_yaw = euler_from_quaternion(self.vector2Arrays(self.sp.pose.orientation))[2]
